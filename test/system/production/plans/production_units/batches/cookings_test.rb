require "application_system_test_case"

module Production::Plans::ProductionUnits
  class Batches::CookingsTest < ApplicationSystemTestCase
    setup do
      sample_cooked_semi_product
      complete_journals @batch
      assert @cooking.update status: :in_progress
      sign_in "daniyar@hey.com"
    end

    test "should create cooking" do
      assert @cooking.destroy

      visit new_production_plan_unit_batch_cooking_url(@plan, @production_unit, @batch)

      accept_alert do
        click_on I18n.t("actions.start")
      end

      assert_text Cooking.enum_to_s(:statuses, :in_progress)
    end

    test "should update Cooking" do
      visit production_plan_unit_batch_cooking_url(@plan, @production_unit, @batch)

      click_on "cooking-menu-button"
      accept_alert do
        click_on I18n.t("actions.finish")
      end

      assert_text Cooking.enum_to_s(:statuses, :completed)
    end
  end
end

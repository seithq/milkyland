module InvitationsHelper
  def button_to_copy_to_clipboard(url, &)
    tag.button class: "icon-btn", data: {
      controller: "copy-to-clipboard", action: "copy-to-clipboard#copy",
      copy_to_clipboard_success_class: "icon-btn-success", copy_to_clipboard_content_value: url
    }, &
  end

  def web_share_button(url, title, text, &)
    tag.button class: "icon-btn", hidden: true, data: {
  ...
  def qr_code_image(url)
    ...
    image_tag qr_code_path(qr_code_link.signed), class: "qr-code", alt: "QR Code"
  end
end

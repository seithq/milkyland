class Packing < ApplicationRecord
  ...
  def build_products
    ...
    products.build(plan.group_products(batch.production_unit.group).map { |product| { packing: self, product: product, count: plan.product_remaining_sum(batch, product) } })
  end
